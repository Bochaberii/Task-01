...
// -----------------------
//
// This file is identical in behavior to the earlier
// wifi_manager.c that lived under main/, but is now part of
// its own reusable component. The includes are written in a
// straightforward way so ESP-IDF's headers can be found
// regardless of where this component is compiled.
...
void update_wifi_credentials(const char *ssid, const char *password)
{
    if (!ssid || !password)
    {
        ESP_LOGW(TAG, "update_wifi_credentials called with null pointer");
        return;
    }
    ...
    {
        ESP_LOGE(TAG, "Failed to save new Wi-Fi credentials to NVS");
        return;
    }

    ESP_LOGI(TAG, "Stored new Wi-Fi credentials in NVS, restarting STA");
    (void)wifi_manager_start_sta(&creds, false);
}
